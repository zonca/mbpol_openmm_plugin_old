...
class CudaCalcMBPolAngleForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolAngleForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MBPolAngleForce& force;
};

CudaCalcMBPolAngleForceKernel::CudaCalcMBPolAngleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
            CalcMBPolAngleForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMBPolAngleForceKernel::~CudaCalcMBPolAngleForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMBPolAngleForceKernel::initialize(const System& system, const MBPolAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    params->upload(paramVector);
    map<string, string> replacements;
    replacements["COMPUTE_FORCE"] = CudaMBPolKernelSources::mbpolAngleForce;
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float2");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMBPolGlobalAngleCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMBPolGlobalAngleQuartic());
    replacements["PENTIC_K"] = cu.doubleToString(force.getMBPolGlobalAnglePentic());
    replacements["SEXTIC_K"] = cu.doubleToString(force.getMBPolGlobalAngleSextic());
...
double CudaCalcMBPolAngleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMBPolAngleForceKernel::copyParametersToContext(ContextImpl& context, const MBPolAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
...
/* -------------------------------------------------------------------------- *
 *                            MBPolInPlaneAngleForce                         *
 * -------------------------------------------------------------------------- */

class CudaCalcMBPolInPlaneAngleForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolInPlaneAngleForce& force) : force(force) {
    }
    int getNumParticleGroups() {
        return force.getNumAngles();
    }
...
private:
    const MBPolInPlaneAngleForce& force;
};

CudaCalcMBPolInPlaneAngleForceKernel::CudaCalcMBPolInPlaneAngleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
          CalcMBPolInPlaneAngleForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMBPolInPlaneAngleForceKernel::~CudaCalcMBPolInPlaneAngleForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMBPolInPlaneAngleForceKernel::initialize(const System& system, const MBPolInPlaneAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
    ...
    map<string, string> replacements;
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float2");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMBPolGlobalInPlaneAngleCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMBPolGlobalInPlaneAngleQuartic());
    replacements["PENTIC_K"] = cu.doubleToString(force.getMBPolGlobalInPlaneAnglePentic());
    replacements["SEXTIC_K"] = cu.doubleToString(force.getMBPolGlobalInPlaneAngleSextic());
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMBPolKernelSources::mbpolInPlaneForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMBPolInPlaneAngleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMBPolInPlaneAngleForceKernel::copyParametersToContext(ContextImpl& context, const MBPolInPlaneAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
...
class CudaCalcMBPolPiTorsionForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolPiTorsionForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MBPolPiTorsionForce& force;
};

CudaCalcMBPolPiTorsionForceKernel::CudaCalcMBPolPiTorsionForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
         CalcMBPolPiTorsionForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMBPolPiTorsionForceKernel::~CudaCalcMBPolPiTorsionForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMBPolPiTorsionForceKernel::initialize(const System& system, const MBPolPiTorsionForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMBPolKernelSources::mbpolPiTorsionForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMBPolPiTorsionForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMBPolPiTorsionForceKernel::copyParametersToContext(ContextImpl& context, const MBPolPiTorsionForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumPiTorsions()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumPiTorsions()/numContexts;
...
class CudaCalcMBPolStretchBendForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolStretchBendForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MBPolStretchBendForce& force;
};

CudaCalcMBPolStretchBendForceKernel::CudaCalcMBPolStretchBendForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
                   CalcMBPolStretchBendForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMBPolStretchBendForceKernel::~CudaCalcMBPolStretchBendForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMBPolStretchBendForceKernel::initialize(const System& system, const MBPolStretchBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMBPolKernelSources::mbpolStretchBendForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMBPolStretchBendForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMBPolStretchBendForceKernel::copyParametersToContext(ContextImpl& context, const MBPolStretchBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumStretchBends()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumStretchBends()/numContexts;
...
class CudaCalcMBPolOutOfPlaneBendForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolOutOfPlaneBendForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MBPolOutOfPlaneBendForce& force;
};

CudaCalcMBPolOutOfPlaneBendForceKernel::CudaCalcMBPolOutOfPlaneBendForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
          CalcMBPolOutOfPlaneBendForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMBPolOutOfPlaneBendForceKernel::~CudaCalcMBPolOutOfPlaneBendForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMBPolOutOfPlaneBendForceKernel::initialize(const System& system, const MBPolOutOfPlaneBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    map<string, string> replacements;
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMBPolGlobalOutOfPlaneBendCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMBPolGlobalOutOfPlaneBendQuartic());
    replacements["PENTIC_K"] = cu.doubleToString(force.getMBPolGlobalOutOfPlaneBendPentic());
    replacements["SEXTIC_K"] = cu.doubleToString(force.getMBPolGlobalOutOfPlaneBendSextic());
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMBPolKernelSources::mbpolOutOfPlaneBendForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMBPolOutOfPlaneBendForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMBPolOutOfPlaneBendForceKernel::copyParametersToContext(ContextImpl& context, const MBPolOutOfPlaneBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumOutOfPlaneBends()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumOutOfPlaneBends()/numContexts;
...
class CudaCalcMBPolTorsionTorsionForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolTorsionTorsionForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MBPolTorsionTorsionForce& force;
};

CudaCalcMBPolTorsionTorsionForceKernel::CudaCalcMBPolTorsionTorsionForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
                CalcMBPolTorsionTorsionForceKernel(name, platform), cu(cu), system(system), gridValues(NULL), gridParams(NULL), torsionParams(NULL) {
}

CudaCalcMBPolTorsionTorsionForceKernel::~CudaCalcMBPolTorsionTorsionForceKernel() {
    cu.setAsCurrent();
    if (gridValues != NULL)
        delete gridValues;
    if (gridParams != NULL)
...
void CudaCalcMBPolTorsionTorsionForceKernel::initialize(const System& system, const MBPolTorsionTorsionForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    for (int i = 0; i < force.getNumTorsionTorsionGrids(); i++) {
        ...
        if (initialGrid[0][0][0] != initialGrid[0][1][0]) {
            MBPolTorsionTorsionForceImpl::reorderGrid(initialGrid, reorderedGrid);
            reordered = true;
        }
    ...
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMBPolKernelSources::mbpolTorsionTorsionForce, replacements), force.getForceGroup());
    cu.getBondedUtilities().addPrefixCode(CudaMBPolKernelSources::bicubic);
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMBPolTorsionTorsionForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}
...
/* -------------------------------------------------------------------------- *
 *                       MBPolGeneralizedKirkwood                            *
 * -------------------------------------------------------------------------- */

class CudaCalcMBPolGeneralizedKirkwoodForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolGeneralizedKirkwoodForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
...
private:
    const MBPolGeneralizedKirkwoodForce& force;
};

CudaCalcMBPolGeneralizedKirkwoodForceKernel::CudaCalcMBPolGeneralizedKirkwoodForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
           CalcMBPolGeneralizedKirkwoodForceKernel(name, platform), cu(cu), system(system), hasInitializedKernels(false), params(NULL), bornRadii(NULL), field(NULL),
           inducedField(NULL), inducedFieldPolar(NULL), inducedDipoleS(NULL), inducedDipolePolarS(NULL), bornSum(NULL), bornForce(NULL) {
}

CudaCalcMBPolGeneralizedKirkwoodForceKernel::~CudaCalcMBPolGeneralizedKirkwoodForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
    if (bornRadii != NULL)
...
void CudaCalcMBPolGeneralizedKirkwoodForceKernel::initialize(const System& system, const MBPolGeneralizedKirkwoodForce& force) {
    cu.setAsCurrent();
    if (cu.getPlatformData().contexts.size() > 1)
        throw OpenMMException("MBPolGeneralizedKirkwoodForce does not support using multiple CUDA devices");
    const MBPolMultipoleForce* multipoles = NULL;
    for (int i = 0; i < system.getNumForces() && multipoles == NULL; i++)
        multipoles = dynamic_cast<const MBPolMultipoleForce*>(&system.getForce(i));
    if (multipoles == NULL)
        throw OpenMMException("MBPolGeneralizedKirkwoodForce requires the System to also contain an MBPolMultipoleForce");
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int paddedNumAtoms = cu.getPaddedNumAtoms();
    int elementSize = (cu.getUseDoublePrecision() ? sizeof(double) : sizeof(float));
    params = CudaArray::create<float2>(cu, paddedNumAtoms, "mbpolGkParams");
    ...
    if (multipoles->getPolarizationType() == MBPolMultipoleForce::Mutual) {
    ...
    for (int i = 0; i < force.getNumParticles(); i++) {
        ...
        // Make sure the charge matches the one specified by the MBPolMultipoleForce.
        ...
        if (charge != charge2)
            throw OpenMMException("MBPolGeneralizedKirkwoodForce and MBPolMultipoleForce must specify the same charge for every atom");
    }
    ...
    if (multipoles->getPolarizationType() == MBPolMultipoleForce::Direct)
        defines["DIRECT_POLARIZATION"] = "";
...
double CudaCalcMBPolGeneralizedKirkwoodForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    // Since GK is so tightly entwined with the electrostatics, this method does nothing, and the force calculation
    // is driven by MBPolMultipoleForce.
    return 0.0;
}

void CudaCalcMBPolGeneralizedKirkwoodForceKernel::computeBornRadii() {
    if (!hasInitializedKernels) {
        ...
        forceSource << CudaMBPolKernelSources::mbpolGk;
        forceSource << "#define F1\n";
        forceSource << CudaMBPolKernelSources::gkPairForce1;
        forceSource << CudaMBPolKernelSources::gkPairForce2;
        forceSource << CudaMBPolKernelSources::gkEDiffPairForce;
...
void CudaCalcMBPolGeneralizedKirkwoodForceKernel::finishComputation(CudaArray& torque, CudaArray& labFrameDipoles, CudaArray& labFrameQuadrupoles,
...
void CudaCalcMBPolGeneralizedKirkwoodForceKernel::copyParametersToContext(ContextImpl& context, const MBPolGeneralizedKirkwoodForce& force) {
    // Make sure the new parameters are acceptable.
...
class CudaCalcMBPolVdwForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolVdwForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
        int iv1, iv2;
...
private:
    const MBPolVdwForce& force;
};

CudaCalcMBPolVdwForceKernel::CudaCalcMBPolVdwForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
        CalcMBPolVdwForceKernel(name, platform), cu(cu), system(system), hasInitializedNonbonded(false), sigmaEpsilon(NULL),
        bondReductionAtoms(NULL), bondReductionFactors(NULL), tempPosq(NULL), tempForces(NULL), nonbonded(NULL) {
}

CudaCalcMBPolVdwForceKernel::~CudaCalcMBPolVdwForceKernel() {
    cu.setAsCurrent();
...
void CudaCalcMBPolVdwForceKernel::initialize(const System& system, const MBPolVdwForce& force) {
    cu.setAsCurrent();
    ...
    if (force.getUseDispersionCorrection())
        dispersionCoefficient = MBPolVdwForceImpl::calcDispersionCorrection(system, force);
    else
    ...
    bool useCutoff = (force.getNonbondedMethod() != MBPolVdwForce::NoCutoff);
    nonbonded->addInteraction(useCutoff, useCutoff, true, force.getCutoff(), exclusions,
        cu.replaceStrings(CudaMBPolKernelSources::mbpolVdwForce2, replacements), force.getForceGroup());
    
    // Create the other kernels.
    
    map<string, string> defines;
    defines["PADDED_NUM_ATOMS"] = cu.intToString(cu.getPaddedNumAtoms());
    CUmodule module = cu.createModule(CudaMBPolKernelSources::mbpolVdwForce1, defines);
    prepareKernel = cu.getKernel(module, "prepareToComputeForce");
    spreadKernel = cu.getKernel(module, "spreadForces");
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMBPolVdwForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
...
void CudaCalcMBPolVdwForceKernel::copyParametersToContext(ContextImpl& context, const MBPolVdwForce& force) {
    // Make sure the new parameters are acceptable.
...
class CudaCalcMBPolWcaDispersionForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MBPolWcaDispersionForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
...
private:
    const MBPolWcaDispersionForce& force;
};

CudaCalcMBPolWcaDispersionForceKernel::CudaCalcMBPolWcaDispersionForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
           CalcMBPolWcaDispersionForceKernel(name, platform), cu(cu), system(system), radiusEpsilon(NULL) {
}

CudaCalcMBPolWcaDispersionForceKernel::~CudaCalcMBPolWcaDispersionForceKernel() {
    cu.setAsCurrent();
    if (radiusEpsilon != NULL)
        delete radiusEpsilon;
}

void CudaCalcMBPolWcaDispersionForceKernel::initialize(const System& system, const MBPolWcaDispersionForce& force) {
    ...
    defines["M_PI"] = cu.doubleToString(M_PI);
    CUmodule module = cu.createModule(CudaKernelSources::vectorOps+CudaMBPolKernelSources::mbpolWcaForce, defines);
    forceKernel = cu.getKernel(module, "computeWCAForce");
    totalMaximumDispersionEnergy = MBPolWcaDispersionForceImpl::getTotalMaximumDispersionEnergy(force);
...
double CudaCalcMBPolWcaDispersionForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int startTileIndex = nb.getStartTileIndex();
    int numTileIndices = nb.getNumTiles();
    int numForceThreadBlocks = nb.getNumForceThreadBlocks();
...
void CudaCalcMBPolWcaDispersionForceKernel::copyParametersToContext(ContextImpl& context, const MBPolWcaDispersionForce& force) {
    // Make sure the new parameters are acceptable.
